...
namespace Nez.Sprites
{
	...
	internal class SpriteAtlasData
	{
		...
		public SpriteAtlas AsSpriteAtlas(Texture2D texture)
		{
			...
			// Copy sprite names and create sprite instances
			atlas.Names = Names.ToArray();
			atlas.Sprites = new Sprite[atlas.Names.Length];

			for (var i = 0; i < atlas.Sprites.Length; i++)
			{
				var size = new Vector2(SourceRects[i].Width, SourceRects[i].Height);
				var pivotPoint = Origins[i] / size;
				atlas.Sprites[i] = new Sprite(texture, SourceRects[i], size, pivotPoint);
			}

			// Copy animation names and create sprite animation instances
